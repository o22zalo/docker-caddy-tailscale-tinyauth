...
function endpoints(source){const map={tailscale:[],cloudflare:[],hybrid:[]};if(source.tailscale?.available&&source.tailscale?.online){const host=source.tailscale.ip||source.tailscale.ips?.[0];if(host)map.tailscale.push({host,port:source.ssh.tailscalePort||2222,proxy:"nc -x tailscale:1055 %h %p"})}if(source.domain)for(let attempt=1;attempt<=3;attempt++)map.cloudflare.push({host:`ssh.${source.domain}`,port:22,attempt,proxy:"docker exec -i cloudflared cloudflared access ssh --hostname %h"});for(const host of source.ssh.ips||[]){if(/^fe80:/i.test(host)||host==="172.17.0.1")continue;map.hybrid.push({host,port:source.ssh.port||22})}return map}
function sshArgs(c,source,channel,auth="key"){return["-o",`BatchMode=${auth==="key"?"yes":"no"}`,"-o",`PasswordAuthentication=${auth==="password"?"yes":"no"}`,"-o","KbdInteractiveAuthentication=no","-o","StrictHostKeyChecking=no","-o",`IdentitiesOnly=${auth==="key"?"yes":"no"}`,"-o",`ConnectTimeout=${cfg.ssh_connect_timeout_seconds||10}`,...(auth==="key"?["-i",keyFile]:[]),"-p",String(c.port),...(c.proxy?["-o",`ProxyCommand=${c.proxy}`]:[])]}
function authModes(){const out=[];if(existsSync(keyFile))out.push({name:"key",cmd:"ssh",prefix:[],env:{}});if(authPass)out.push({name:"password",cmd:"sshpass",prefix:["-e","ssh"],env:{SSHPASS:authPass}});return out}
