...
function endpoints(source){const map={tailscale:[],cloudflare:[],hybrid:[]};if(source.tailscale?.available&&source.tailscale?.online){const host=source.tailscale.ip||source.tailscale.ips?.[0];if(host)map.tailscale.push({host,port:source.ssh.tailscalePort||2222,proxy:"nc -x tailscale:1055 %h %p"})}if(source.domain)for(let attempt=1;attempt<=3;attempt++)map.cloudflare.push({host:`ssh.${source.domain}`,port:22,attempt,proxy:"docker exec -i cloudflared cloudflared access ssh --hostname %h"});for(const host of source.ssh.ips||[]){if(/^fe80:/i.test(host))continue;map.hybrid.push({host,port:source.ssh.port||22})}return map}
